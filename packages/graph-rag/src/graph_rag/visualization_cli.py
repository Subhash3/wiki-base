...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render a Graph RAG node-link JSON file as interactive HTML."
    )
...
def visualize_file(json_file: Path) -> Path:
    ...
    network = GraphVisualizer.from_json(json_file.read_text(encoding="utf-8"))
    html_file = json_file.with_suffix(".html")
    html_file.write_text(
        GraphVisualizer.to_html(network, title=f"Graph RAG · {json_file.stem}"),
        encoding="utf-8",
    )
    return html_file
